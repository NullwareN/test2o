...
class BotManager {
    ...
    record_ipc_query_failure(reason) {
        ...
        const now = Date.now();
        if (!this.last_ipc_failure_log || now - this.last_ipc_failure_log > 15000 || this.consecutive_ipc_failures <= 3) {
            this.last_ipc_failure_log = now;
